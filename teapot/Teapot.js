/**
 * @fileoverview Creates teapot mesh from obj file. Code adapted from CS418 Lab exercise 
 * @author Eric Schaffer
 */

/**
 * Creates teapot from obj mesh file.
 */
class Teapot {
    /**
     * Initialise members of teapot object
     */
    constructor(){
        // Keep track of whether obj file has been loaded
        this.isLoaded = false;
        
        // Used for webgl to know how many times draw to screen.
        this.numFaces=0;
        this.numVertices=0;
    ...
    */
    ...
    /**
     * Handles comments in the OBJ File.
     * @param {String} line contains a comment
     */
    handleComment(line) {
        console.log(line);
    }
    ...
    /**
     * Handles vertex lines in the OBJ File.
     * @param {String} line contains vertex data
     */
    handleVertex(line) {
        var vertex_data = line.split(' ');
        this.vBuffer.push(parseFloat(vertex_data[1]));
        this.vBuffer.push(parseFloat(vertex_data[2]));
        this.vBuffer.push(parseFloat(vertex_data[3]));
        this.numVertices += 1;
    }
    
    /**
     * Handles comments in the OBJ File.
     * @param {String} line contains face data
     */
    handleFace(line) {
        var face_data = line.split(' ');

        // Vertex indices for each face are 1 indexed.
        this.fBuffer.push(parseInt(face_data[1] - 1));
        this.fBuffer.push(parseInt(face_data[2] - 1));
        this.fBuffer.push(parseInt(face_data[3] - 1));
        this.numFaces += 1
    }
    ...
    */
    loadFromOBJ(fileText) {
        ...
        for (var i = 0; i < lines.length; i++) {
            if (lines[i][0] == '#') {
                this.handleComment(lines[i]);
            } else if (lines[i][0] == 'v') {
                this.handleVertex(lines[i]);
            } else if (lines[i][0] == 'f') {
                this.handleFace(lines[i]);
            }
        }
        //----------------
